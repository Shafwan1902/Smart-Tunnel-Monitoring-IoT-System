...
# --- HARDWARE SETUP (From your smart tunnel.py) ---
# LEDs
...
while True:
    try:
        ...
        # 2. SAFETY LOGIC (Immediate Response)
        ...
        # 3. PREPARE JSON DATA
        # We send the exact values + the calculated status
        payload = {
        ...
        print(f"Sent: {status} | Dist: {dist_cm:.1f}cm | Water: {water_val}")

    except Exception as e:
        print("Error:", e)
        # Optional: Reconnect MQTT if needed
        # try: client.connect()
        # except: pass

    time.sleep(1) # Fast update (1 second)